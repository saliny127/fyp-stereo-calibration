...
def fit_polynomial(leftx, lefty, rightx, righty, size_binary_warped_0):
    ...
    # size_binary_warped_0 - max(min(lefty),min(righty))
    ind_0 = max(min(lefty), min(righty))
    # print(ind_0)
...
def draw_line(out_img, left_fitx, right_fitx, ploty):
    ...
    draw_points_left = (np.asarray([left_fitx, ploty]).T).astype(
        np.int32)   # needs to be int32 and transposed
    draw_points_right = (np.asarray([right_fitx, ploty]).T).astype(
        np.int32)   # needs to be int32 and transposed
    # args: image, points, closed, color
    cv2.polylines(out_img, [draw_points_left], False, (0, 255, 0), 7)
    # args: image, points, closed, color
    cv2.polylines(out_img, [draw_points_right], False, (0, 255, 0), 7)
...
def visualize_region_search_around_poly(binary_warped, left_lane_inds, right_lane_inds, left_fitx, right_fitx, margin_around_line, ploty, nonzeroy, nonzerox):
    ...
    left_line_window1 = np.array(
        [np.transpose(np.vstack([left_fitx-margin_around_line, ploty]))])
    left_line_window2 = np.array([np.flipud(np.transpose(np.vstack([left_fitx+margin_around_line,
                                                                    ploty])))])
    left_line_pts = np.hstack((left_line_window1, left_line_window2))
    right_line_window1 = np.array(
        [np.transpose(np.vstack([right_fitx-margin_around_line, ploty]))])
...
def visualize_window_serach(binary_warped_window_pixel, undist_road, Minv, img_size):
    ...
    binary_warped_window_pixel_unwraped = unwarp(
        binary_warped_window_pixel, Minv, img_size)
    ...
    road_window = cv2.addWeighted(
        undist_road, 1., binary_warped_window_pixel_unwraped, 0.8, 0.)
...
def visualize_perspective_transfor(undist_road, src):
    ...
    road_rectangale = cv2.line(
        undist_road, (src[0, 0], src[0, 1]), (src[1, 0], src[1, 1]), (0, 255, 0), 2)
    road_rectangale = cv2.line(
        road_rectangale, (src[3, 0], src[3, 1]), (src[2, 0], src[2, 1]), (0, 255, 0), 2)
    road_rectangale = cv2.line(
        road_rectangale, (src[0, 0], src[0, 1]), (src[3, 0], src[3, 1]), (0, 255, 0), 2)
    road_rectangale = cv2.line(
        road_rectangale, (src[1, 0], src[1, 1]), (src[2, 0], src[2, 1]), (0, 255, 0), 2)
...
def find_lane(image):
    ...
    undist_road = cv2.undistort(image, mtx, dist, None, mtx)
    undist_road = image
    ...
    binary_warped_window_pixel = visualize_detected_pixels(
        binary_warped_window, lefty, leftx, righty, rightx)
    ...
    binary_warped_window_pixel_line = draw_line(
        binary_warped_window_pixel, left_fitx, right_fitx, ploty)
    ...
    left_curverad, right_curverad = measure_curvature_real(
        ploty, left_fitx, right_fitx, ym_per_pix, xm_per_pix)
    ...
    car_off_center = measure_off_center_real(
        left_fitx[0], right_fitx[0], img_size[0], xm_per_pix)
    ...
    road_lane = visualize_lane(
        binary_warped, undist_road, ploty, left_fitx, right_fitx, Minv, img_size)
...
while (cap.isOpened()):
    ...
    if cv2.waitKey(1) & 0xFF == ord('a'):
        find_lane(frame_left, frame_right)
